...
**태스크:**
- [x] FastAPI 앱 초기화, FAISS 인덱스 + 메타데이터 서버 시작 시 로드
- [x] `POST /search/text` 엔드포인트 (통합 `POST /search`로 구현됨)
- [x] `POST /search/image` 엔드포인트 (통합 `POST /search`로 구현됨)
- [x] K1+K2 통합 정렬 로직: 양쪽 결과 병합 후 `score` 내림차순 정렬, 중복 출원번호 처리(최고 점수 유지)
- [x] `GET /` → `static/index.html` 서빙
- [x] CORS 설정 (개발 중 `*` 허용)
...
**태스크:**
- [x] 이미지 업로드 UI: `<input type="file">` + 미리보기
- [x] 텍스트 입력 UI: `<textarea>` + K1/K2 개수 선택 드롭다운 (2x2 그리드로 고도화)
- [x] 검색 버튼: 이미지·텍스트 중 입력된 항목만 API 호출 (둘 다 가능)
- [x] 결과 3컬럼 렌더링:
  - K1 리스트: 출원번호, 발명명칭, 청구항 요약, 유사도 점수
  - K3 리스트: 출원번호, 구성요소, 유사도 점수
  - K2 리스트: 출원번호, 도면번호, 도면 썸네일, 유사도 점수
  - K1&K2 통합: 출처 태그 포함, 유사도 내림차순
- [x] 로딩 스피너 (API 응답 대기 중)
- [x] 에러 메시지 표시
- [x] **추가**: 초기화 버튼 및 고해상도 엑셀 추출 기능
